...
const AddEditDispenser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await dispenserService.update(patientData);
        toast.success("User updated successfully!");
      } else {
        await dispenserService.register(patientData);
  ...
  return (
    <form onSubmit={handleSubmit} className="w-[90%] m-auto">
      <h1 className="m-[30px] text-center font-[700] text-[20px]">
        {id ? "Edit Dispenser" : "Add Dispenser"}
      </h1>
      <Divider />
      <div>
...
export default AddEditDispenser;
